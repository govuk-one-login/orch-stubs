import { decrypt } from "./decryption-helper.ts";
import * as keyHelper from "./key-helpers.ts";
...
describe("KMS decryption service", () => {
  ...
  it("should return a jwt", async () => {
    vi.spyOn(keyHelper, "getContentEncryptionKey").mockResolvedValueOnce(
      new Uint8Array([
        142, 95, 23, 226, 246, 50, 142, 180, 28, 234, 156, 229, 103, 214, 179,
        219, 246, 139, 213, 206, 155, 126, 217, 196, 129, 23, 198, 54, 32, 219,
        115, 38,
      ])
    );
  ...
  it("should throw a CodedError with error details when an invalid encryption key is used", async () => {
    ...
    await expect(action()).rejects.toThrow(
      "Error decrypting JWE The operation failed for an operation-specific reason"
    );
  });
});
